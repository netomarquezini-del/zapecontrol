...
export default function MarketingDocumentosPage() {
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      const resp = await fetch('/api/marketing/documentos');
      if (!resp.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const resp = await fetch(`/api/marketing/documentos?id=${id}`, {
        method: 'DELETE',
      });
  ...
  const { getDragProps, isOver, isDragging } = useDragReorder({
    ...
    apiEndpoint: '/api/marketing/documentos',
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-extrabold text-white">Documentos Marketing</h1>
          <p className="text-xs text-zinc-500 mt-1">
            Gerencie os documentos de marketing da equipe
          </p>
        </div>
