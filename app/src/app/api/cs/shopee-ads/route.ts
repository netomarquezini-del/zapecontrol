...
const SHOPEE_ADS_GROUP_NAMES: Record<string, string> = {
  '120363422457783091-group': 'Shopee ADS 2.0 | Neto Marquezini',
  '120363407332110646-group': 'Shopee ADS 2.0 | Neto Marquezini #001',
  '120363407280170820-group': 'Shopee ADS 2.0 | Neto Marquezini #002',
  '120363404311146540-group': 'Shopee ADS 2.0 | Neto Marquezini #002',
  '120363424726740000-group': 'Shopee ADS 2.0 | Neto Marquezini #003',
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Parallel queries
    const [msgsRes, analysisRes] = await Promise.all([
      supabase
        .from('cs_messages')
        .select('id, sender_name, sender_phone, is_team_member, content, message_type, timestamp, group_id')
        .in('group_id', SHOPEE_ADS_GROUP_IDS)
    ...
    const groupName = 'Shopee ADS 2.0 | Neto Marquezini'

    // ─── Per-group breakdown ───
    const perGroup: Record<string, { name: string; messages: number; members: Set<string> }> = {}
    for (const m of messages) {
      const gid = m.group_id
      if (!gid) continue
      if (!perGroup[gid]) {
        perGroup[gid] = {
          name: SHOPEE_ADS_GROUP_NAMES[gid] || gid,
          messages: 0,
          members: new Set(),
        }
      }
      perGroup[gid].messages++
      if (!m.is_team_member) {
        perGroup[gid].members.add(m.sender_phone || m.sender_name)
      }
    }

    const group_breakdown = SHOPEE_ADS_GROUP_IDS.map((id) => ({
      id,
      name: perGroup[id]?.name || SHOPEE_ADS_GROUP_NAMES[id] || id,
      messages: perGroup[id]?.messages || 0,
      members: perGroup[id]?.members.size || 0,
    }))
    ...
    console.error('[CS Shopee ADS] Error:', e.message)
