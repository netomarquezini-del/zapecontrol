...
creative_pipeline:
  ...
  remotion_engine:
    script: "squads/criativoset/criativos/render-static-ad.js"
    compositions:
      - id: "static-ad-square"
...
integration:
  input_sources:
    ...
    swipe:
      ...
      via: "/squads/criativoset/criativos/swipe/"

  output_destinations:
    criativos:
      path: "/squads/criativoset/criativos/novos/"
...
templates:
  criativo-duelo:
    path: "/root/zapeecomm/squads/criativoset/templates/criativo-duelo.md"
    descricao: "Formato comparativo Certo vs Errado — split 50/50, 4 itens por lado, 1024x1024"
    render: "gemini-2.5-flash-image (fallback: gpt-image-1)"
    nota: "Quando receber pedido de criativo certo-errado, CARREGAR este template e seguir specs exatos"
  criativo-duelo02:
    path: "/root/zapeecomm/squads/criativoset/templates/criativo-duelo02.md"
    descricao: "Formato comparativo Certo vs Errado Hero Icon — split 50/50, 1 item por lado, icones 3D grandes, fundo tech, 1080x1350"
    render: "gemini-2.5-flash-image (fallback: gpt-image-1)"
    nota: "Versao impactante do Duelo — icones 3D como hero, fundo tech/digital, headline scroll-stop"
...
mission-control:
  skills:
    - name: Design de Criativos Estaticos
      description: Cria anuncios estaticos para Meta Ads com composicao visual profissional — tipografia, cores, layout e hierarquia visual.
    - name: Geracao de Imagens IA
      description: Usa Gemini para imagens premium, Flux para volume e Ideogram para imagens com texto integrado. Seleciona o provider ideal.
    - name: Tipografia e Composicao
      description: Aplica regras de tipografia (fonte grande, imponente) com hierarquia clara — headline, corpo, CTA. Albert Sans sempre.
    - name: Scoring Automatico
      description: Avalia cada criativo gerado em 10 dimensoes (contraste, legibilidade, hierarquia, etc) e atribui score 0-100.
    - name: Creative Packs em Escala
      description: Produz pacotes de 10-20 variacoes de um criativo variando cores, layouts e imagens para teste A/B em escala.
    - name: 24 Modelos Mentais
      description: Biblioteca de 24 frameworks de design e persuasao visual que guiam cada decisao criativa — de Gestalt a neurodesign.
  crons:
    - name: Static Pipeline (from Max)
      description: Pipeline automatico que recebe briefings do Max e inicia producao de criativos estaticos
      schedule: Sob demanda (trigger)
      hours: Quando Max finaliza briefing
      days: Variavel
      what_it_does: >
        Monitora a tabela creative_briefings no Supabase. Quando Max insere
        um novo briefing com status=ready, Thomas recebe e inicia producao —
        gera imagens via Gemini/Flux, aplica tipografia, compoe o layout,
        roda scoring automatico e salva creative pack pronto para o Leo subir.
      on_failure: Marca briefing como error, notifica Max para revisar
      status: scheduled
  integrations:
    - name: Google Gemini
      purpose: Geracao de imagens premium com alta fidelidade — usado para hero shots e imagens principais
    - name: Replicate
      purpose: Geracao de imagens em volume via Flux Schnell — usado para testes rapidos e variacoes
    - name: Ideogram
      purpose: Geracao de imagens com texto integrado — ideal para criativos que precisam de texto na imagem
    - name: Supabase
      purpose: Tracking de criativos gerados, scores de qualidade e integracao com Leo para upload
